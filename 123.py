...
class SimpleHTTPRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if 'subject' in  issue_information:
            cursor.execute('SELECT last_subject FROM last_subject_list')
            result = cursor.fetchall()
            subject_last = result[0][0]
            if subject_last != issue_information['subject']:
                ...
                cursor.execute(
                    'INSERT INTO last_subject_list (last_subject) VALUES (%s)',
                    (issue_information['subject']))
